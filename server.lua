...
---------------------
            ...
            MySQL.Async.execute('INSERT INTO user_policeArmory  (identifier, weapons, PlayerName) VALUES (@identifier, @weapons, @PlayerName)', {
...
ESX.RegisterServerCallback("esx_policeArmory:checkPoliceOnline", function(source, cb)
    MySQL.Async.fetchAll('SELECT * FROM user_policeArmory', {}, function(rowsChanged)
        ...
        for k,v in pairs(rowsChanged) do
            ...
            if xPlayer ~= nil then
                table.insert(police,{id = v.identifier,name = v.PlayerName,job = xPlayer.getJob()})
                print(v.PlayerName) 
            end
        end
        cb(police)
